...
app.setupSections = function(){
	...
	var publishNews = function(){
		return new Promise(function(resolve,reject){
			var checkForm = utils.checkForm('.wem_nm__sections');
	...
	// setTimeout(function(){
		if (!$content.hasClass('isNew')) {
			var checkAll = utils.checkForm($content);
			if(checkAll.valid){
				...
				$content.find('.wem_nm__section').each(function(){
					...
					if ((parseInt($section.attr('data-step'))+1) != $content.find('.wem_nm__section').length) {
						var check = utils.checkForm($section);
						if (check.valid){
...
app.checkFormValidity_ = function checkFormValidity(container) {
